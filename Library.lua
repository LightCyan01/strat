...
-- // seed mechanic (per-session coordinate variance)
local SeedValue = (LocalPlayer.UserId + os.time()) % 2147483647
local function NextSeed()
    SeedValue = (SeedValue * 1103515245 + 12345) % 2147483647
    return SeedValue
end

local function ApplySeed(pos)
    local ox = (NextSeed() % 9 + 2) * 1e-10
    local oy = (NextSeed() % 9 + 2) * 1e-10
    local oz = (NextSeed() % 9 + 2) * 1e-10
    if NextSeed() % 2 == 0 then ox = -ox end
    if NextSeed() % 2 == 0 then oy = -oy end
    if NextSeed() % 2 == 0 then oz = -oz end
            ...
            local ok, res = pcall(function()
                ...
                if AbData then
                    ...
                    if positions and #positions > 0 then
                        data.towerPosition = ApplySeed(positions[math.random(#positions)])
                    end
...
function TDS:Place(TName, px, py, pz, ...)
    ...
    DoPlaceTower(TName, ApplySeed(Vector3.new(px, py, pz)))
